...
import React, { useState, useEffect } from 'react';
 ...
 export default function getAllRestaurants() {
  ...
  useEffect(() => {
    ...
    fetch('https://trendy-taste-a7jv.onrender.com/restaurant/getAllRestaurants')
      ...
      <table className="w-full table-striped rounded-sm p-2.5 border-separate table-auto border-red-700  shadow-red-700 shadow-lg">
        <thead className="">
          <tr className="bg-red-600 font-semibold text-3xl">
            ...
            <th className="border px-4 py-2">Restaurant Image</th>
          </tr>
        </thead>
        <tbody>
          {data.map((user, index) => (
            <tr key={user._id} className="font-semibold text-2xl">
              ...
              <td className="border px-4 py-2">{user.restaurantName}</td>
              <td className="border px-4 py-2">{user.location}</td>
              <td className="border px-4 py-2">{user.restaurantImg}</td>
            </tr>
          ))}
        </tbody>
      </table>
